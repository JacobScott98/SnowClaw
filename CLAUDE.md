...
| Command | Description |
|---------|-------------|
| `snowclaw setup` | Interactive wizard: collects credentials, writes .env + openclaw.json + connections.toml, optionally creates Snowflake objects via REST API |
...
`install.sh` clones the repo to `~/snowclaw` (override with `SNOWCLAW_DIR`), installs pipx if needed, then runs `pipx install -e` from the local checkout. Re-running does `git pull` + reinstall. No PyPI involved.

...
def scaffold_project(target: Path, force: bool = False) -> tuple[list[str], list[str]]:
    ...
    # Set executable bit on shell scripts (inline during copy would require
    # a second check; rglob is fine for a one-time setup operation)
...
def apply_prefix(root: Path, prefix: str):
    """Replace __SNOWCLAW_PREFIX__ placeholders in scaffolded template files."""
    suffixes = {".yaml", ".sql", ".sh"}
    for f in root.rglob("*"):
        if f.is_file() and f.suffix in suffixes:
            content = f.read_text()
            if "__SNOWCLAW_PREFIX__" in content:
                f.write_text(content.replace("__SNOWCLAW_PREFIX__", prefix))
...
def cmd_setup(args: argparse.Namespace):
    ...
    # --- Apply prefix to scaffolded templates ---
    apply_prefix(root, prefix)
    ...
    (root / ".snowclaw").write_text(json.dumps(marker, indent=2) + "\n")
